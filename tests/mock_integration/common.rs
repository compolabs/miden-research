...
pub fn get_custom_account_code(
    ...
    let account_code_src = include_str!("../../src/partial_fill_swap/user_wallet.masm");
...
pub fn create_partial_swap_note(
    ...
    let note_code = include_str!("../../src/partial_fill_swap/SWAPp.masm");
...
pub fn create_consumer_creator_test(
    ...
    let note_code = include_str!("../../src/reclaim/is_consumer_creator.masm");
    let (note_script, _code_block) = new_note_script(
        ProgramAst::parse(note_code).unwrap(),
        &TransactionKernel::assembler().with_debug_mode(true),
    )
